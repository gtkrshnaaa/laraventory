...
@section('content')
    <div class="py-6">
        <div class="mx-auto max-w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
            <!-- List -->
            <div class="lg:col-span-2 bg-white/80 backdrop-blur-sm border-2 border-gray-200/50 rounded-2xl overflow-hidden">
                            ...
                            <div class="flex items-start justify-between space-x-4">
                                <div class="flex-1">
                                    <div class="flex items-center space-x-3 mb-2">
                                        <div class="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center border-2 border-gray-200/50">
                                ...
                                <div class="flex items-center space-x-2">
                                    <div x-data="{ editing: false }" class="flex items-center space-x-2">
                                        <template x-if="!editing">
                                            <button @click="editing = true" class="group relative p-2 text-blue-600 hover:text-blue-800 bg-blue-50/30 hover:bg-blue-100/50 rounded-lg border-2 border-gray-200/30 hover:border-gray-300/50 transition-all duration-200 transform hover:-translate-y-0.5" title="Edit">
                                        ...
                                        <template x-if="editing">
                                            <form action="{{ route('admin.categories.update', $cat->id) }}" method="POST" class="flex items-center space-x-2">
                                                @csrf
                                                @method('PUT')
                                                <input type="text" name="name" value="{{ $cat->name }}" class="px-3 py-2 bg-white/80 backdrop-blur-sm border-2 border-transparent rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Nama" required>
                                                <input type="text" name="description" value="{{ $cat->description }}" class="px-3 py-2 bg-white/80 backdrop-blur-sm border-2 border-transparent rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Deskripsi">
                                                <button type="submit" class="group relative p-2 text-emerald-600 hover:text-emerald-800 bg-emerald-50/30 hover:bg-emerald-100/50 rounded-lg border-2 border-emerald-200/50 hover:border-emerald-300/50 transition-all duration-200 transform hover:-translate-y-0.5" title="Simpan">
                                                    <i data-lucide="check" class="w-4 h-4"></i>
                                                </button>
                                                <button type="button" @click="editing = false" class="p-2 text-gray-500 hover:text-gray-700 bg-white/80 hover:bg-gray-100 rounded-lg border-2 border-gray-200/50 hover:border-gray-300/50 transition-all duration-200 transform hover:-translate-y-0.5" title="Batal">
                                    ...
                                    <form action="{{ route('admin.categories.destroy', $cat->id) }}" method="POST" onsubmit="return confirm('Hapus kategori ini?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="relative group p-2 text-white rounded-lg transition-all duration-200 transform hover:-translate-y-0.5 bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 shadow-sm shadow-blue-200/50 hover:shadow-blue-200" title="Hapus">
                ...
                <div class="p-6">
                    <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" class="w-full px-4 py-2.5 text-sm bg-white/80 backdrop-blur-sm border-2 border-gray-200/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-gray-300 transition-all duration-200 hover:border-gray-300/50 placeholder-gray-400" placeholder="Contoh: AC & Pendingin" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Deskripsi</label>
                            <textarea name="description" class="w-full px-4 py-2.5 text-sm bg-white/80 backdrop-blur-sm border-2 border-gray-200/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-gray-300 transition-all duration-200 hover:border-gray-300/50 placeholder-gray-400 resize-none" rows="3" placeholder="Deskripsi kategori (opsional)"></textarea>
                        </div>
